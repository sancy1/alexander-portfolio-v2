...
// File: services/auth-service/AuthService.Application/Features/Admin/Commands/UpdateAdminProfileHandler.cs
...
public sealed class UpdateAdminProfileHandler : IRequestHandler<UpdateAdminProfileCommand, UpdateAdminProfileResponse>
{
    ...
    public async Task<UpdateAdminProfileResponse> Handle(UpdateAdminProfileCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrEmpty(request.Username) && request.Username != admin.Username)
        {
            ...
            changes.Add($"username: {admin.Username} → {request.Username}");
        }

        // Check if email is taken (if changing)
        string currentEmailValue = admin.Email; // Resolved: Extracted primitive value directly
        if (!string.IsNullOrEmpty(request.Email) && request.Email.ToLowerInvariant() != currentEmailValue)
        {
            var existing = await _adminRepository.GetByEmailAsync(request.Email);
            if (existing != null)
            {
            ...
            changes.Add($"email: {currentEmailValue} → {request.Email}");
        }

        // 1. Mutate tracking status parameters strictly inside in-memory state lines
        admin.UpdateProfile(request.Username, request.Email?.ToLowerInvariant());
        _adminRepository.Update(admin);

        // 2. Queue outbox message entities inside the same tracking frame ONLY if data changes occurred
        if (changes.Any())
        {
            var profileUpdatePayload = new
            {
                username = admin.Username,
            ...
            await OutboxHelper.AddToOutboxAsync(
                _outboxRepository,
                eventType: "admin.profile.updated",
                routingKey: "admin.profile.updated",
                broker: "rabbitmq",
        ...
        // 🏗️ Admin updates and outbox lines commit in one singular PostgreSQL transaction block
        await _unitOfWork.SaveChangesAsync(cancellationToken);
