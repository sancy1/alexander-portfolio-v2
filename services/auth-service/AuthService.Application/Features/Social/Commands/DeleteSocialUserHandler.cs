...
// File: services/auth-service/AuthService.Application/Features/Social/Commands/DeleteSocialUserHandler.cs
...
public sealed class DeleteSocialUserHandler : IRequestHandler<DeleteSocialUserCommand, DeleteAccountResponse>
{
    ...
    public async Task<DeleteAccountResponse> Handle(DeleteSocialUserCommand request, CancellationToken cancellationToken)
    {
        ...
        // 🧠 Resolved: Access standard string primitive directly without .Value extension wrapper
        string userEmailValue = user.Email ?? string.Empty;
        if (!userEmailValue.Equals(request.ConfirmEmail, StringComparison.OrdinalIgnoreCase))
        {
            return new DeleteAccountResponse { Success = false, Message = "Email confirmation does not match" };
        }
        ...
        var deletionPayload = new
        {
            accountId = user.Id,
            accountType = "social_user",
            email = userEmailValue, // Serializes clean string value
            displayName = user.DisplayName,
            provider = user.Provider.ToString().ToLowerInvariant(),
            reason = request.Reason,
        ...
        if (request.PermanentDelete)
        {
            ...
            await OutboxHelper.AddToOutboxAsync(
                ...
                routingKey: "social.user.permanently_deleted",
                broker: "kafka",
            ...
            await OutboxHelper.AddToOutboxAsync(
                _outboxRepository,
                eventType: "social.user.permanently_deleted",
                routingKey: "social.user.permanently_deleted",
                broker: "rabbitmq",
                payload: deletionPayload,
                userId: user.Id.ToString(),
            ...
            // 🏗️ Single Atomic Transaction Commit
            await _unitOfWork.SaveChangesAsync(cancellationToken);
            
            // ☁️ Execute external media cleanup outside critical DB transaction blocks
            if (!string.IsNullOrEmpty(avatarUrlSnapshot) && 
                !avatarUrlSnapshot.Contains("googleusercontent", StringComparison.OrdinalIgnoreCase) && 
                !avatarUrlSnapshot.Contains("github", StringComparison.OrdinalIgnoreCase))
            {
        ...
        else
        {
            ...
            await OutboxHelper.AddToOutboxAsync(
                _outboxRepository,
                eventType: "social.user.soft_deleted",
                routingKey: "social.user.soft_deleted",
                broker: "rabbitmq",
                payload: deletionPayload,
                userId: user.Id.ToString(),
                userType: "social-user");
