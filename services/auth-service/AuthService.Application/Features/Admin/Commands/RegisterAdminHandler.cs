...
// File: services/auth-service/AuthService.Application/Features/Admin/Commands/RegisterAdminHandler.cs
...
public sealed class RegisterAdminHandler : IRequestHandler<RegisterAdminCommand, AuthResponse>
{
    ...
    public async Task<AuthResponse> Handle(RegisterAdminCommand request, CancellationToken cancellationToken)
    {
        ...
        // 3. Track Intent Locally (DbContext Tracker Queue)
        await _adminRepository.AddAsync(adminEntity);

        // Standardized inner payload body mapping
        var registrationPayload = new
        {
            username = adminEntity.Username,
            email = adminEntity.Email, // Resolved: Extracted primitive value directly
            role = adminEntity.Role.ToString().ToLowerInvariant(),
        ...
        await OutboxHelper.AddToOutboxAsync(
            _outboxRepository,
            eventType: "admin.registered",
            routingKey: "admin.registered",
            broker: "rabbitmq",
            payload: registrationPayload,
            userId: adminEntity.Id.ToString(),
            userType: "admin");
        ...
        // 🏗️ Single Atomic Transaction Commit Rule
        await _unitOfWork.SaveChangesAsync(cancellationToken);

        // 🧠 Post-Commit Security Token Generation
        var token = _jwtGenerator.GenerateAdminToken(adminEntity);
