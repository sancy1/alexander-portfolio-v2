...
// File: services/auth-service/AuthService.Application/Features/Admin/Commands/DeleteAccountHandler.cs
...
public sealed class DeleteAccountHandler : IRequestHandler<DeleteAccountCommand, DeleteAccountResponse>
{
    ...
    public async Task<DeleteAccountResponse> Handle(DeleteAccountCommand request, CancellationToken cancellationToken)
    {
        ...
        var deletionPayload = new
        {
            ...
            accountType = "admin",
            username = admin.Username,
            email = admin.Email, // Resolved: Extracted primitive value directly
            reason = request.Reason,
        ...
        if (request.PermanentDelete)
        {
            // Using standardized EventEnvelope for Kafka (Permanent Chronological Timeline Log)
            await OutboxHelper.AddToOutboxAsync(
                _outboxRepository,
                eventType: "auth-events",
                routingKey: "admin.account.permanently_deleted",
                broker: "kafka",
                payload: deletionPayload,
                userId: admin.Id.ToString(),
                userType: "admin");

            // Using standardized EventEnvelope for RabbitMQ (Real-time notification engine)
            await OutboxHelper.AddToOutboxAsync(
                _outboxRepository,
                eventType: "admin.account.permanently_deleted",
                routingKey: "admin.account.permanently_deleted",
                broker: "rabbitmq",
                payload: deletionPayload,
                userId: admin.Id.ToString(),
        ...
        else
        {
            ...
            await OutboxHelper.AddToOutboxAsync(
                _outboxRepository,
                eventType: "admin.account.soft_deleted",
                routingKey: "admin.account.soft_deleted",
                broker: "rabbitmq",
                payload: deletionPayload,
                userId: admin.Id.ToString(),
                userType: "admin");

            // Using standardized EventEnvelope for Kafka (Permanent compliance logging)
            await OutboxHelper.AddToOutboxAsync(
                _outboxRepository,
